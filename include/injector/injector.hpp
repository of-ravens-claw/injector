 ...
 */
inline bool ProtectMemory(memory_pointer_tr addr, size_t size, SceUInt32 protection)
{
    return sceKernelMprotect(addr.get(), size, (int)protection) == SCE_OK;
}
 ...
 */
inline bool UnprotectMemory(memory_pointer_tr addr, size_t size, SceUInt32& out_oldprotect)
{
    // VM_PROT_ALL doesn't exist as a constant, so we use the value manually
    out_oldprotect = 7; // hardcoding to VM_PROT_ALL, since we can't retrieve the actual value (afaik)
    return sceKernelMprotect(addr.get(), size, 7) == SCE_OK;
}
...
inline bool game_version_manager::Detect()
{
    ...
    // We don't care about this, just pretend there's no game.
    return false;
}
